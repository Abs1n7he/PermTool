        ...
        if self.LogSwitch == True:
            ...
            if not os.path.isfile(self.logFile):
                with open(self.logFile, 'a', encoding='utf-8') as log:
                    ...
                    log.close()
            with open(self.logFile, 'a', encoding='utf-8') as log:
                log.write(time.strftime("<details><summary>[%Y/%m/%d %H:%M:%S] ",time.localtime(time.time())) + ecd(url) +
                          " ["+ecd(self.headerTxt)+"]</summary>" +
                          "<div class=\"div1\"><p>" + ecd(req_1).replace('\n', '<br>') + '<br>' +
                          ''.join([ecd(key + ': ' + value) + '<br>' for key, value in headers.items()]) +
                          '<br>' + ecd(req_2).replace('\n', '<br>') + "</p><br>")
                log.write('<p>[response:]<br>' + str(status) + " " + reason + "<br>" +
                          ''.join([ecd(key + ':' + value) + '<br>' for key, value in dict_res_header.items()]) + "<br><span>" +
        ...
        if self.objectName().startswith('zhushi'):
            ...
            if req and headers:
                ...
                if 'Content-Type' in tempheaders and 'json' in tempheaders['Content-Type']:
                    try:
                        jsonBody = str(''.join([i.strip() for i in body.split('\n')]))
...
class Example(QMainWindow):
    def __init__(self):
        ...
        try:
            ...
            show_popup('Error', 'config文件不规范，yaml格式错误')
            config={}
    ...
    def initUI(self):
        ...
        try:
            ...
            show_popup('Error', 'config文件不规范，读取内容失败')
    ...
    def saveConfig_windows(self):
        ...
        self.new_window.setWindowIcon(self.icon)
        ...
        self.saveConfig_btn = QPushButton("保存", clicked=lambda: self.saveConfig(self.save_log.isChecked(),self.save_proxy.isChecked(),self.save_onlyCookies.isChecked(),
            self.save_header1.isChecked(),self.save_header2.isChecked(),self.save_header3.isChecked(),self.save_header4.isChecked(),self.save_header5.isChecked(),self.save_header6.isChecked(),
            self.save_req.isChecked(),self.save_req_header.isChecked(),self.save_res_header.isChecked()))
    ...
    def saveConfig(self,t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12):
        data={
        'Log': False,
        'proxy': 'http://127.0.0.1:8080',
        'onlyCookies': '筛选请求头:',
        ...
        if t1:
            data['Log']=self.LogSwitch
        if t2:
            data['proxy']=self.proxy.text()
        if t3:
            data['onlyCookies']=self.onlyCookies.text()
        if t4:
            data['header1']=str_to_json(self.header1.toPlainText())
        if t5:
            data['header2']=str_to_json(self.header2.toPlainText())
        if t6:
            data['header3']=str_to_json(self.header3.toPlainText())
        if t7:
            data['header4']=str_to_json(self.header4.toPlainText())
        if t8:
            data['header5']=str_to_json(self.header5.toPlainText())
        if t9:
        ...
        if t12:
            data['check_res_header']=str_to_json(self.check_res_header.toPlainText())
